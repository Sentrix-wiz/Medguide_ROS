...
# Workspace root — scripts/ is at <ws>/src/medguide_robot/scripts/
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
WS = os.path.abspath(os.path.join(_SCRIPT_DIR, '..', '..', '..'))
...
class ExperimentOrchestrator(Node):
    ...
    def _do_launch(self, response):
        ...
        def _wait_for_ready():
            ...
            for i in range(60):  # Max 60 seconds
                ...
                # Keep asserting initial pose every 2 seconds until localized
                if not self.localized and i >= 3 and i % 2 == 1:
                    pub.publish(msg)
            ...
            if self.stack_running:
                self.mode = 'IDLE'
                self.get_logger().warn('AMCL timeout — assuming ready')
            self.destroy_publisher(pub)
    ...
    def _do_shutdown(self, response):
        ...
        if self._launch_proc and self._launch_proc.poll() is None:
            try:
                pgid = os.getpgid(self._launch_proc.pid)
    ...
    def _do_teleop(self, response):
        """Enable manual teleop control via Dashboard."""
        ...
        self.mode = 'TELEOP'
        response.success = True
        response.message = 'Teleop mode active (use UI D-Pad)'
        self.get_logger().info('🎮 Teleop active via Dashboard UI')
        return response
